    ...
    async updateImage(req:Request, res:Response){
        if(req.params.id){
            try {
                ...
                const video = (await new ItemService().getVideoById(id)).data;
                
                if(video.actorName !== userToken.userName)
                ...
                const pictures = await imageService.findImage(id ,'video');
                if(pictures === null)
                    return statusResponse.sendResponseJson(
                        CodeStatut.NOT_FOUND_STATUS,
                        res,
                        `Aucune image associer à cette video !`
                    );
